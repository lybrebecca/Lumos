...
function MonthBlock({ habits, year, month, isFuture, onAddLog }) {
  ...
  return (
    <div style={{
      ...
      >
        <div style={{ minWidth: NAME_WIDTH + days * CELL_SIZE }}>
                  ...
                  >
                    {count > 0 ? (
                      <div
                        onContextMenu={e => {
                          e.preventDefault()
                          if (!isFutureDay && !isFuture) {
                            onAddLog(habit, year, month, d)
                          }
                        }}
                        style={{
                          width: '20px',
...
function StatsPage() {
  ...
  function confirmAddLog() {
    ...
    if (isArchived) {
      const { saveArchivedHabits } = require('../utils/storage')
      ...
      const { saveHabits } = require('../utils/storage')
